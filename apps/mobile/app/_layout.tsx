...
function Navigation() {
  ...
  useEffect(() => {
    ...
    const openRecharge = (source: string, requestId: string) => {
      ...
      router.replace({ pathname: "/(tabs)/(tokens)", params: { recharge: requestId, rechargeSource: source, autoUnlock: "1" } } as never);
    };
    const checkPendingRecharge = async (source = "shield", openTokensWhenStale = false) => {
      const pending = await restrictionEngine.hasPendingIntervention().catch(() => null);
      if (pending) openRecharge(source, pending);
      else if (openTokensWhenStale) router.replace("/(tabs)/(tokens)" as never);
    };
    const subscription = restrictionEvents?.addListener("onInterventionRequested", () => void checkPendingRecharge("native-event"));
    const appStateSubscription = AppState.addEventListener("change", (state) => {
      if (state === "active") void checkPendingRecharge("foreground");
    });
    const notificationSubscription = Notifications.addNotificationResponseReceivedListener((response) => {
      if (response.notification.request.content.data?.route === "tokens") void checkPendingRecharge("notification", true);
    });
    void Notifications.getLastNotificationResponseAsync().then((response) => {
      if (response?.notification.request.content.data?.route !== "tokens") return;
...
export default function RootLayout() {
  const [fontsLoaded] = useFonts({ Recursive_400Regular, Recursive_500Medium, Recursive_600SemiBold, Recursive_700Bold });
