alter table public.reward_intents add column idempotency_key text;
create unique index reward_intents_user_idempotency_idx
  on public.reward_intents (user_id, idempotency_key);
...
begin
  ...
  from public.reward_intents
  where user_id = p_user_id and state = 'provisional' and verified_at is null;
  if pending_count >= 3 then
    raise exception 'pending_reward_limit_reached';
  end if;
