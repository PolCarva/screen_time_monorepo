...
- Usage Access (`PACKAGE_USAGE_STATS`) is requested by the onboarding story and by Today's "Use my real usage" (docs/onboarding-v2-plan.md D4, docs/real-savings-estimate-plan.md D8-D9). It is optional ("Continue without my data") and read in one file, `StillUsageInsights.kt`. Both entry points show the same prominent disclosure before Settings: what is read (how much each app is used), what for (showing the user their own time and estimating the time they get back) and that it is worked out and kept on the phone and never leaves it.
- What is kept, on the phone only: per-app totals (sessions, median session length, seconds per day) for the week before the onboarding and the last seven days, in the app's SQLite kv-store, plus minutes per skipped pause per app in `noBackupFilesDir/session-minutes.json`. Android Auto Backup copies only SharedPreferences (`secure_store_backup_rules.xml`, `secure_store_data_extraction_rules.xml`), so none of this leaves the device through backups either. Nothing is sent to Still's servers or analytics: `/api/v1/wellbeing/daily` still carries only Still's own pause counters and the server keeps the config's minutes per avoided open. Data safety does not declare usage data as collected.
