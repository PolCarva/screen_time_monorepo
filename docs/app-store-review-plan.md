...
| Paso a 0.3.7 en ASC | hecho (2026-09-26, pedido del usuario) | La versión `f79f8af8…` ahora es **0.3.7** con la build 20 (`PREPARE_FOR_SUBMISSION`, TestFlight interno `IN_BETA_TESTING`); el envío `ad546c6a…` sigue en `UNRESOLVED_ISSUES` hasta U3. Notas (3818 caracteres) desde `docs/store-listing.md`: la espera del anuncio pasa a 3 s, después de la respiración se elige el tiempo, y entra el párrafo de anuncios que faltaba. La línea «NUNCA QUEDAS BLOQUEADO» de las descripciones es-MX/en-US ya no dice 5 minutos. Capturas sin cambios (no muestran la espera). La respuesta de §11.1 ya dice 0.3.7 y 3 s. |
