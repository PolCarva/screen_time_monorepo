...
- Android uses a disclosed Accessibility service, a local app picker, and monotonic unlock deadlines tied to the current boot. Today counts Still's own pauses per local day, the same on Android and iOS; Still does not ask for Usage Access.
...
- Impact pages use only persisted Supabase data. Missing or unavailable data is shown explicitly; production UI never substitutes demo totals. The weekly fund is live: each rewarded ad AdMob confirms adds its estimated value, and AdMob's daily report replaces the estimate once the day settles. The pages also show the ads watched, the people who funded the week, and the people and time Still gave back ([plan](docs/real-impact-stats-plan.md)).
